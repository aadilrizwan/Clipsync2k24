...
io.on('connection', (socket) => {
    ...
    socket.on('process-video', async (data) => {
        ...
        fs.readFile(filePath, async (err, file) => {
            ...
            if (processing.data.status !== 200) {
                return console.error("Error: Something went wrong with creating the processing file");
            }
            ...
            cloudinary.uploader.upload(filePath, {
                ...
                if (error) {
                    ...
                    console.log("Video has been uploaded successfully to Cloudinary:", result.secure_url);

                    // Process further based on the plan (if user has "PRO" plan)
                    if (processing.data.plan === "PRO") {
                        fs.stat(filePath, async (err, stat) => {
                            if (!err && stat.size < 25000000) { // File smaller than 25MB
                                const transcription = await cloudinary.uploader.transcribe(filePath, {
                                ...
                                if (transcription) {
                                    ...
                                    if (titleAndSummaryGenerated.data.status !== 200) {
                                        console.log("Something went wrong while creating the title and description");
                                    }
                                }
                            }
                        });
